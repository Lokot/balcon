...
import ru.skysoftlab.balcon.ui.OrdersView;
...
public class ChatView extends CustomComponent implements View {
	...
	private static final long serialVersionUID = 8356568215045339205L;
	...
	private Layout buildUserLayout() {
		...
		layout.addComponent(generateOrderButton());
		return layout;
	}

	private Component generateOrderButton() {
		...
		button.addClickListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				navigationEvent.fire(new NavigationEvent(Conventions
						.deriveMappingForView(OrdersView.class)));
			}
		});
		return button;
	}
		...
		if (targetUser != null && message.involves(currentUser, targetUser)) {
			if (messageLayout != null) {
				if (messageLayout.getComponentCount() >= MAX_MESSAGES) {
					messageLayout.removeComponent(messageLayout.iterator()
							.next());
				}
