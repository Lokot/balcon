package ru.skysoftlab.balcon.web;
...
import ru.skysoftlab.balcon.qualifier.SimpleQualifierQualifier;
...
public class UniversalResource extends HttpServlet {
	...
	private static final Logger log = Logger.getLogger(UniversalResource.class);
	...
	private UniversalAction commonHandle(HttpServletRequest request, HttpServletResponse response) {
		...
		if (qualifier == null || qualifier.equals("")) {
			log.errorf("Parameter 'qualifier' not defined in %s HTTP Request", request.getMethod());
			return null;
		}
	...
	private UniversalAction getUniversalAction(final String qualifier) {
		...
		} catch(UnsatisfiedResolutionException | IllegalArgumentException e) {
			log.errorf("No UniversalAction bean with qualifier = '%s' found !", qualifier);
			return null;
		}		
	}
